...
inpath = rootPath + 'Setup2_UG_RF_1050V_90d_60d_240nsCoMPASS_300_200lsb_15h/RAW/SDataR_Setup2_UG_RF_1050V_90d_60d_240nsCoMPASS_300_200lsb_15h.CSV'
...
def plotCoinsSimple(df, outpath=inpath, coinWin_ns=[-100., 100.], titExtra="NO OFFLINE CUTS", nameExtra="000", chLst=[0, 2, 4]):
    ''' Plots information about coincidences without the reordered file
    '''
    ri = runInfo(outpath)
...
#    axx[0][0].set_xticks(range(multMax+1), labels=[str(i) for i in range(multMax+1)])
    ...
    for ext in ['.pdf']: plt.savefig(outpath + "_Coins_" + str(nameExtra) + ext)
    plt.show(); plt.clf()
...
def sortCoins(inPath=inpath, coinWin_ns=[-100., 100.], plot=True):
    ...
    if plot: plotCoinsSimple(dfc, outpath=ri.outPath, coinWin_ns=coinWin_ns, titExtra="NO OFFLINE CUTS", nameExtra="000", chLst=[0, 2, 4])
...
def getChLst(df):
    ...
    chLst = (list(set([((col.replace("E","")).split("_AU")[0]) for col in df.columns if "_AU" in col]))); chLst.sort()
    return chLst
...
def plotSortedDataQuality(dfl, ri, eRng=None, log=False):
    ...
    for i, ch in enumerate(chLst):
        ...
        ax[1][i].hist(dfc['T' + str(ch) + '_ps']*ps_s, bins=tBins_s, weights=[1.0/tStep_s]*len(dfc['T' + str(ch) + '_ps']), histtype='step', color='b', lw=3, rasterized=True, label='E' + str(ch) + '>0')
        ...
        for och, col in zip(chLst[i+1:], ['r', 'g']):
            ...
            ax[1][i].hist(dfcc['T' + str(ch) + '_ps']*ps_s, bins=tBins_s, weights=[1.0/tStep_s]*len(dfcc['T' + str(ch) + '_ps']), histtype='step', color=col, lw=3, rasterized=True, label='E' + str(ch) + '>0, E'+ str(och) + '>0')
...
def getSortedData(ri, plot=True):
    ...
    #
    if plot: 
        plotSortedDataQuality(dfl, ri, eRng=None, log=False)
    #    plotSortedDataStability(df, ri, mainCh=0, eRng=[[0., 600.], [0., 3000.], [0., 2000.]], log=False)
    #    plotSortedData(df, ri, mainCh=0, eRng=[[0., 600.], [0., 3000.], [0., 2000.]], log=False)
...
if __name__ == '__main__':
    ...
    ip10 = rootPath + 'Setup2_UG_RF_1000V_80d_50d_240nsCoMPASS_1000_200lsb_15h/FILTERED/SDataF_Setup2_UG_RF_1000V_80d_50d_240nsCoMPASS_1000_200lsb_15h.CSV'
    ip11 = rootPath + 'Setup2_UG_RF_1000V_80d_50d_240nsCoMPASS_1000_200lsb_15h/RAW/SDataR_Setup2_UG_RF_1000V_80d_50d_240nsCoMPASS_1000_200lsb_15h.CSV'
    ...
    ip30 = rootPath + 'Setup2_UG_RF_1050V_90d_60d_240nsCoMPASS_300_200lsb_15h/RAW/SDataR_Setup2_UG_RF_1050V_90d_60d_240nsCoMPASS_300_200lsb_15h.CSV'
    ip31 = rootPath + 'Setup2_UG_RF_1000V_80d_50d_240nsCoMPASS_1000_200lsb_NoSource_5h_1/RAW/SDataR_Setup2_UG_RF_1000V_80d_50d_240nsCoMPASS_1000_200lsb_NoSource_5h_1.CSV'
    ...
    if True:
        ...
        #for inpath in [ip30]:
            ...
            if False: plotCoinsSimple(dfc, outpath=inpath, coinWin_ns=[-cw_ns, cw_ns], titExtra="NO OFFLINE CUTS", nameExtra="000", chLst=[0, 2, 4])
            ...
            if False: getSortedData(ri, plot=True)
