...
class Hists2D:
    def __init__(self,ch,Energy,nbins):
        ...
        self.data = {f"{ch[0]}":Energy[0],
                     f"{ch[1]}":Energy[1]}
        self.hist2D = TH2D(f"{channels[f'Channel {self.ch[0]}'][0]} vs {channels[f'Channel {self.ch[1]}'][0]} 2D Histogram", f"{channels[f'Channel {self.ch[0]}'][0]} vs {channels[f'Channel {self.ch[1]}'][0]}", self.nbins, 0, 4000, self.nbins, 0, 4000)
        
        self.hist2DInverse = TH2D(f"{channels[f'Channel {self.ch[1]}'][0]} vs {channels[f'Channel {self.ch[0]}'][0]} 2D Histogram", f"{channels[f'Channel {self.ch[1]}'][0]} vs {channels[f'Channel {self.ch[0]}'][0]}", self.nbins, 0, 4000, self.nbins, 0, 4000)
        for i in range(len(Energy[0])):
            self.hist2D.Fill(Energy[0][i],Energy[1][i])
            self.hist2DInverse.Fill(Energy[1][i],Energy[0][i])
            
        self.profile = None
        self.profileInverted = None
        self.cut = None  
        self.fit = None  
    ...
    def Draw2DHistXY(self,saveFilePath):
        self.canvas = TCanvas(f"canvas_ch{self.ch[0]}_vs_ch{self.ch[1]}", "canvas", 2400, 1000)
        self.canvas.Divide(2,1)
        
        ROOT.gStyle.SetPalette(ROOT.kBlueGreenYellow)
        
        pad = self.canvas.cd(1) #Plot the left subplot first
        ...
        self.hist2D.GetZaxis().SetTitleOffset(1.5) 
        ...
        if self.fit is not None:
            ...
            legend = TLegend(0.17, 0.65, 0.60, 0.88)
        ...
        pad = self.canvas.cd(2) #Plot the right subplot second
        # pad.SetLeftMargin(0.15)  
        # pad.SetRightMargin(0.15)  
        ...
        self.hist2DInverse.GetXaxis().SetTitle(f"{channels[f'Channel {self.ch[1]}'][0]} Energy (ADU)")
        self.hist2DInverse.GetYaxis().SetTitle(f"{channels[f'Channel {self.ch[0]}'][0]} Energy (ADU)")
        self.hist2DInverse.GetZaxis().SetTitle("Counts")
        self.hist2DInverse.GetZaxis().SetTitleOffset(1.8) 
        
        self.hist2DInverse.Draw("COLZ")
        self.hist2DInverse.SetStats(0)
        if self.cut is not None:
            self.cutInverted.Draw("same")
        if self.profile is not None:
            self.profileInverted.SetStats(0) 
            self.profileInverted.Draw("same")
        if self.fit is not None:
            self.fitInverted.Draw("same")
            
            # Extract fit results
            self.p0Inv       = self.fitInverted.GetParameter(0)
            self.p0Inv_err   = self.fitInverted.GetParError(0)
            self.p1Inv       = self.fitInverted.GetParameter(1)
            self.p1Inv_err   = self.fitInverted.GetParError(1)
            self.chi2Inv     = self.fitInverted.GetChisquare()
            self.ndofInv     = self.fitInverted.GetNDF()
            self.p_valueInv  = Stats.chi2.sf(self.chi2Inv, self.ndofInv)  # survival function = 1 - CDF
            
            # Build legend — positioned to the left of the colour bar
            # (x1, y1, x2, y2) in NDC coordinates (0-1)
            legend = TLegend(0.35, 0.65, 0.78, 0.88)
            legend.SetHeader("Fit Results (pol1)", "C")
            legend.AddEntry(self.fitInverted, f"p0 = {self.p0Inv:.2f} #pm {self.p0Inv_err:.2f}", "l")
            legend.AddEntry((0), f"p1 = {self.p1Inv:.4f} #pm {self.p1Inv_err:.4f}", "")
            legend.AddEntry((0), f"#chi^{{2}} / ndof = {self.chi2Inv:.2f} / {self.ndofInv}", "")
            legend.AddEntry((0), f"p-value = {self.p_valueInv:.4f}", "")
            legend.SetBorderSize(1)
            legend.SetFillColorAlpha(0, 0)  # semi-transparent background
            self.legendInverted = legend               # store to prevent garbage collection
            self.legendInverted.Draw()
        self.canvas.Update()
        self.canvas.SaveAs(str(saveFilePath / f"Ch_{self.ch[0]}_vs_Ch_{self.ch[1]}_X_Y_profile_histogram.png"))
        self.canvas.SaveAs(str(saveFilePath / f"Ch_{self.ch[0]}_vs_Ch_{self.ch[1]}_X_Y_profile_histogram.pdf"))    
    ...
    def saveData(self,filepath, outputFileName):
        ...
        if self.profile is not None and self.profileInverted is None:
            outputFile = filepath / f"{outputFileName}_Profile_hist_fit_results.txt"
        ...
        elif self.profile is not None and self.profileInverted is not None:
            outputFile = filepath / f"{outputFileName}_Profile_hist_fit_results_X_and_Y_Hist.txt"
        
            writeHeader = not outputFile.exists()
            with open(outputFile, "a") as f:
                if writeHeader:
                    f.write('Date,ch1,ch2,fit func, fit range, Inv fit range, chi^2, nDOF, P-value, P0, P0Err, P1, P1Err, Inv chi^2, Inv nDOF, Inv P-value, Inv P0, Inv P0Err, Inv P1,  Inv P1Err \n')
                f.write(f'{date}, {self.ch[0]}, {self.ch[1]},Pol1, [{self.fitRange[0]}_{self.fitRange[1]}],[{self.fitRangeInverted[0]}_{self.fitRangeInverted[1]}], {self.chi2},{self.ndof},{self.p_value},{self.p0},{self.p0_err},{self.p1},{self.p1_err}, {self.chi2Inv},{self.ndofInv},{self.p_valueInv},{self.p0Inv},{self.p0Inv_err},{self.p1Inv},{self.p1Inv_err}\n')
        else:
            print('Fit Data is not present to write to disk. ')
        
        f.close()
        
    def cutHist(self,cutEndPoints, cutEndPointsInverted = None):
        ...
        if cutEndPointsInverted is not None:
            self.cutInverted = TCutG(f'Banana_cut_{channels[f'Channel {self.ch[1]}'][0]}_vs_{channels[f'Channel {self.ch[0]}'][0]}', 4)
            for i in range(len(cutEndPointsInverted)):
                self.cutInverted.SetPoint(i,cutEndPointsInverted[i][0], cutEndPointsInverted[i][1])
            self.cutInverted.SetLineColor(kRed+2)
            self.cutInverted.SetLineWidth(3)
    ...
    def makeProfileHistXY(self,scaleFactor):
        ...
        self.cutHist2DInverted = TH2D( f"cut_hist_ch_{self.ch[1]}_vs_ch_{self.ch[0]}",
        f"Cut Histogram Ch {self.ch[1]} vs Ch {self.ch[0]}",
        self.nbins, 0, 4000, self.nbins, 0, 4000)
        
        self.profile = TProfile(f"profile_ch_{channels[f'Channel {self.ch[0]}'][0]}_vs_ch_{channels[f'Channel {self.ch[1]}'][0]}",f"profile Histogram {channels[f'Channel {self.ch[0]}'][0]} vs {channels[f'Channel {self.ch[1]}'][0]}", self.nbins,0,4000,0,4000,"")
        
        self.profileInverted = TProfile(f"profile_ch_{channels[f'Channel {self.ch[1]}'][0]}_vs_ch_{channels[f'Channel {self.ch[0]}'][0]}",f"profile Histogram {channels[f'Channel {self.ch[1]}'][0]} vs {channels[f'Channel {self.ch[0]}'][0]}", self.nbins,0,4000,0,4000,"")
        
        
        
        ##############################################################################################################################
        # Fill the normal cut histogram
        ##############################################################################################################################
        ...
        ##############################################################################################################################
        # Fill the Inverted cut histogram
        ##############################################################################################################################
        
        maxBin = 0
        for bini in range(1, self.hist2DInverse.GetNbinsX() + 1):
            for binj in range(1, self.hist2DInverse.GetNbinsY() + 1): 
                x = self.hist2DInverse.GetXaxis().GetBinCenter(bini)
                y = self.hist2DInverse.GetYaxis().GetBinCenter(binj)
                z = self.hist2DInverse.GetBinContent(bini,binj)
                
                if not self.cutInverted.IsInside(x,y) and z > 0:
                    # self.profile.Fill(x,y,z)
                    if z > maxBin:
                        maxBin = z
        ...
        for bini in range(1, self.hist2DInverse.GetNbinsX() + 1):
            for binj in range(1, self.hist2DInverse.GetNbinsY() + 1): 
                ...
                if not self.cut.IsInside(x,y) and z > cutThreshold:
                    self.profileInverted.Fill(x,y,z) #For this method we are filling the profile directly from the cut histogram
                    self.cutHist2DInverted.Fill(x,y,z)
        ...
        self.fitRangeInverted = [min(xBins),max(xBins)]
        ...
        ###################################################################################################################################################################
        # Fit the inverted profile
        ###################################################################################################################################################################
        self.fitInverted = TF1(f"fit_{channels[f'Channel {self.ch[1]}'][0]}_vs_{channels[f'Channel {self.ch[0]}'][0]}","pol1",self.fitRangeInverted[0], self.fitRangeInverted[1])
        self.profileInverted.Fit(self.fitInverted, "RN")
        self.fitInverted.SetLineColor(kRed)
        
        
        ###################################################################################################################################################################
        # Plot the two cut histograms on the same canvas
        ###################################################################################################################################################################
        ...
        self.cutCanvas.Divide(2,1)
        
        self.cutCanvas.cd(1) #Selects the first subplot (the left one)
        ...
        self.cutCanvas.cd(2) #Moves to the second subplot.
        
        self.cutHist2DInverted.SetStats(0)
        self.cutHist2DInverted.GetXaxis().SetTitle(f"Channel {self.ch[1]} Energy (keV)")
        self.cutHist2DInverted.GetYaxis().SetTitle(f"Channel {self.ch[0]} Energy (keV)")
        self.cutHist2DInverted.GetZaxis().SetTitle("Counts")
        self.cutHist2DInverted.Draw("COLZ")
        self.cutCanvas.Update()
...
for filePath in coincFiles:
    ...
    for data in hist2DData:
        ...
        if XYProfile:
            data.cutHist(cutEndPoints,cutEndPointsInverse)
