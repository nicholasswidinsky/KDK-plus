...
rootPath = '/Users/arnaudlemaire/ECL/Queens/Experiment/LS/ComptonNewDAQ/pizza/'

inpath = rootPath + 'pizza_60_90_Zn65/RAW/SDataR_pizza_60_90_Zn65.CSV'
...
def plotCoinsSimple(df, ri, coinWin_ns=[-100., 100.], titExtra="NO OFFLINE CUTS", nameExtra="000", chLst=[0, 2, 4]):
    ''' Plots information about coincidences without the reordered file
    '''
    #ri = runInfo(outpath)
...
def sortCoins(inPath=inpath, coinWin_ns=[-100., 100.], plot=True):
    ...
    if plot: plotCoinsSimple(dfc, ri=ri, coinWin_ns=coinWin_ns, titExtra="NO OFFLINE CUTS", nameExtra="000", chLst=[0, 2, 4])
...
def plotSortedSpectraQuality(dfl, ri, eRng=None, okFlags=['0x4000', '0x4100'], log=False, eRanges_AU=None, tRanges_ns=None):
...
if __name__ == '__main__':
    ...
    ip10 = rootPath + '/Setup2_UG_RF_1050V_90d_60d_240nsCoMPASS_300_200lsb_15h/FILTERED/SDataF_Setup2_UG_RF_1050V_90d_60d_240nsCoMPASS_300_200lsb_15h.CSV'
    ip11 = rootPath + '/Setup2_UG_RF_1050V_90d_60d_240nsCoMPASS_300_200lsb_15h/RAW/SDataR_Setup2_UG_RF_1050V_90d_60d_240nsCoMPASS_300_200lsb_15h.CSV'
    ...
    # New setup, FILT and RAW
    ip30 = rootPath + 'pizza_60_90_Zn65/FILTERED/SDataF_pizza_60_90_Zn65.CSV'
    ip31 = rootPath + 'pizza_60_90_Zn65/RAW/SDataR_pizza_60_90_Zn65.CSV'
    
    # Lower HV, FILT and RAW
    subFolder = '/Setup2_UG_RF_1000V_80d_50d_240nsCoMPASS_1000_200lsb_15h/'
    ...
    if True:
        ...
        for inpath in [ip30,ip31]:
        #for inpath in [ip30]:
        #for inpath in [ip40, ip41]:
            
            ### ID coins
            if True: ri = runInfo(fullInFile=inpath)
            if True: df, dfCh, inpath = loadData(inpath=inpath, verbose=True, plot=True, drop=True, nrows=nMax)
                
            # if False: timeDiffs(df, dfCh, outpath=inpath, plot=True) # Deprecated
            # if False: dfc = findCoins(df, outpath=inpath, coinWin_ns=cw_ns, back=True, verbose=False, plot=True, cStart=0, clean=True) # Deprecated
            if True: dfc = sortCoins(inPath=inpath, coinWin_ns=[-cw_ns, cw_ns], plot=True)    
            if True: plotCoinsSimple(dfc, outpath=inpath, coinWin_ns=[-cw_ns, cw_ns], titExtra="NO OFFLINE CUTS", nameExtra="000", chLst=[0, 2, 4])
            # if False: plotCoinsBasic(dfc, outpath=inpath, coinWin_ns=cw_ns) # Deprecated
            # if False: plotCoins(dfc, outpath=inpath, coinWin_ns=cw_ns) # Deprecated

            ### Work with IDed data
            # if False: loadSortedData(ri, plot=True) # Deprecated
            if True: res = getSortedData(ri, plot=False)
            #if False: plotSortedSpectraQuality(res['Data coins'], ri, eRng=None, okFlags=['0x4000', '0x4100'], log=False, eRanges_AU=None, tRanges_ns=None)
            if True: plotSortedSpectraQuality(res['Data coins'], ri, eRng=None, okFlags=['0x4000', '0x4100'], log=False, eRanges_AU=[[0, 1000], [0, 3000], [0, 3000]], tRanges_ns=[[-240, 0], [-240, 240], [-240, 240]])
...
# rootPath = "/Users/distefano/Documents/Data/TestData/CAEN_Test/2SiPM_coincidence_80d_50d_NoCoincBoard_240nsCoMPASS_CoincidenceWindow_1200_200lsb_15h_raw/"

# #inPath = rootPath + "/FILTERED/SDataF_2SiPM_coincidence_80d_50d_240nsBoard_240nsCoMPASS_CoincidenceWindow_1200_200lsb_15h_raw.CSV"
# inPath = rootPath + "/RAW/SDataR_2SiPM_coincidence_80d_50d_NoCoincBoard_240nsCoMPASS_CoincidenceWindow_1200_200lsb_15h_raw.CSV"

# #sortPath = rootPath + "/FILTERED/SDataF_2SiPM_coincidence_80d_50d_240nsBoard_240nsCoMPASS_CoincidenceWindow_1200_200lsb_15h_raw_COINSORTED.CSV"
# sortPath = rootPath + "/RAW/SDataR_2SiPM_coincidence_80d_50d_NoCoincBoard_240nsCoMPASS_CoincidenceWindow_1200_200lsb_15h_raw_COINSORTED.CSV"

# compCmd = "g++ -o sortCAENdata sortCAENdata.cpp"
# runCmd = "./sortCAENdata" + " " + inPath + " " + sortPath

# cw_ns = [-100., 100.]

# if False:
#     compValue = os.system(compCmd)  # returns the exit code in unix
#     print('Cmd:', compCmd, 'returned value:', compValue)

#     runValue = os.system(runCmd)  # returns the exit code in unix
#     print('Cmd:', runCmd, 'returned value:', runValue)
# if True:
#     #a = subprocess.check_output(compCmd)
#     a = subprocess.run(["g++", "-o", "sortCAENdata", "sortCAENdata.cpp"], capture_output=True)
#     print(a)
#     #b = subprocess.check_output(runCmd)
#     b = subprocess.run(["./sortCAENdata", inPath, sortPath, str(cw_ns[0]), str(cw_ns[1])], capture_output=True)
#     print(b)

# dfIn = pd.read_csv(inPath, sep=";")
# print(dfIn)

# dfOut = pd.read_csv(sortPath, sep="\t")
# print(dfOut)

# colLgths = [len(dfOut[dfOut['T' + str(i) + '_ps']>0]['T' + str(i) + '_ps']) for i in range(6)]
# print(colLgths, np.sum(colLgths))

# #print(dfOut[dfOut['Flag0']=='0x4100'])

# for i in [0, 2, 4]:
#     plt.hist(dfOut['E' + str(i) + '_AU'], bins=70, log=True, histtype='step', color='b', lw=6)
#     plt.hist(dfIn[dfIn['CHANNEL']==i]['ENERGY'], bins=70, log=True, histtype='step', color='r', lw=2)
#     plt.show(); plt.clf()
    
# for ii in [[0,2], [2,4], [4,0]]:
#     plt.hist2d(dfOut[((dfOut['E' + str(ii[0]) + '_AU']>0) & (dfOut['E' + str(ii[1]) + '_AU']>0))]['E' + str(ii[0]) + '_AU'], 
#                dfOut[((dfOut['E' + str(ii[0]) + '_AU']>0) & (dfOut['E' + str(ii[1]) + '_AU']>0))]['E' + str(ii[1]) + '_AU'], 
#                bins=70, norm=matplotlib.colors.LogNorm())
#     plt.show(); plt.clf()
    
#     plt.hist((dfOut[((dfOut['E' + str(ii[0]) + '_AU']>0) & (dfOut['E' + str(ii[1]) + '_AU']>0))]['T' + str(ii[0]) + '_ps'] - dfOut[((dfOut['E' + str(ii[0]) + '_AU']>0) & (dfOut['E' + str(ii[1]) + '_AU']>0))]['T' + str(ii[1]) + '_ps']) * ps_ns, bins=100)
#     plt.show(); plt.clf()
